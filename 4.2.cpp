...
void orderer(int** mass, int inum, int* jnum, int* order)
{
	...
	int counter = 0;
	int jnumm = *jnum;
	for (int j = 0; j < jnumm; j++)
	{
		int minjter;
		for (int l = 0; l < jnumm; l++)
		{
			if (l == 0)
			{
				minjter = l;
		...
		if (elcounter[minjter] != 0)
		{
			order[counter] = minjter;
			counter++;
		}
		else
		{
...
глобальные переменные в них использовать нельзя.
	2.	Доделать задачу 1 так, чтобы в матрице остались строки только с ненулевой характеристикой*/
